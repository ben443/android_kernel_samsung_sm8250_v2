...
DTS_DIR=$(pwd)/arch/$ARCH/boot/dts
...
cp $(pwd)/defconfig $(pwd)/arch/$ARCH/configs/vendor/x1q_chn_openx_defconfig
exit 0
fi

cp $(pwd)/arch/$ARCH/boot/Image $(pwd)/Image

DTBO_FILES=$(find ${DTS_DIR}/samsung/ -name ${CHIPSET_NAME}-sec-*-r*.dtbo)

cat ${DTS_DIR}/vendor/qcom/*.dtb > $(pwd)/dtb.img
